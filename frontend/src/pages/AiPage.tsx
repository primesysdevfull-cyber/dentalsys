...
export function AiPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">IA / Automação</h1>
        <p className="text-gray-500 dark:text-gray-400">Transcrição de voz e sugestão inteligente de prontuário</p>
      </div>
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Gerar Sugestão</h3>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Paciente *</label>
                <select value={patientId} onChange={(e) => setPatientId(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100 focus:border-dental-500 focus:outline-none">
                  <option value="">Selecione...</option>
                  {patients?.data?.map((p: any) => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Procedimento realizado</label>
                <input type="text" value={procedureName} onChange={(e) => setProcedureName(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100" placeholder="Ex: Restauração dente 26" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">Observações / Sintomas</label>
                <textarea value={notes} onChange={(e) => setNotes(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100" rows={4} placeholder="Descreva os sintomas ou observações do paciente..." />
              </div>
          ...
          <div className="rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Sugestão</h3>
            ...
            {suggestion && editableSuggestion ? (
              <div className="space-y-3 text-sm">
                <div className="rounded-lg bg-blue-50 p-3">
                  ...
                  <textarea value={editableSuggestion.suggestedDiagnosis} onChange={(e) => setEditableSuggestion({ ...editableSuggestion, suggestedDiagnosis: e.target.value })} className="w-full rounded border border-blue-200 bg-white dark:bg-gray-800 px-2 py-1 text-sm dark:text-gray-100 focus:border-blue-500 focus:outline-none" rows={2} />
                </div>
                <div className="rounded-lg bg-green-50 p-3">
                  <p className="text-xs font-medium text-green-700 mb-1">Procedimento</p>
                  <textarea value={editableSuggestion.suggestedProcedure} onChange={(e) => setEditableSuggestion({ ...editableSuggestion, suggestedProcedure: e.target.value })} className="w-full rounded border border-green-200 bg-white dark:bg-gray-800 px-2 py-1 text-sm dark:text-gray-100 focus:border-green-500 focus:outline-none" rows={2} />
                </div>
                <div className="rounded-lg bg-orange-50 p-3">
                  <p className="text-xs font-medium text-orange-700 mb-1">Prescrição</p>
                  <textarea value={editableSuggestion.suggestedPrescription} onChange={(e) => setEditableSuggestion({ ...editableSuggestion, suggestedPrescription: e.target.value })} className="w-full rounded border border-orange-200 bg-white dark:bg-gray-800 px-2 py-1 text-sm dark:text-gray-100 focus:border-orange-500 focus:outline-none" rows={2} />
                </div>
                <div className="rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                  <p className="text-xs font-medium text-gray-700 dark:text-gray-400 mb-1">Observações</p>
                  <textarea value={editableSuggestion.suggestedObservations} onChange={(e) => setEditableSuggestion({ ...editableSuggestion, suggestedObservations: e.target.value })} className="w-full rounded border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-2 py-1 text-sm dark:text-gray-100 focus:border-gray-500 focus:outline-none" rows={3} />
                </div>
                {suggestion.disclaimer && <p className="text-xs italic text-gray-400 dark:text-gray-500">{suggestion.disclaimer}</p>}
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Transcrever Áudio</h3>
            </div>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-400">URL do Áudio</label>
                <input type="text" value={audioUrl} onChange={(e) => setAudioUrl(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-200 dark:border-gray-600 px-3 py-2 text-sm dark:bg-gray-800 dark:text-gray-100" placeholder="https://..." />
                <p className="mt-1 text-xs text-gray-400 dark:text-gray-500">Link público do arquivo de áudio (MP3, WAV, M4A)</p>
              </div>
          ...
          <div className="rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-900 p-6 shadow-sm">
            <div className="mb-4 flex items-center gap-2">
              ...
              <h3 className="font-semibold text-gray-900 dark:text-gray-100">Como funciona</h3>
            </div>
            <div className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
              ...
              <div className="mt-4 rounded-lg bg-gray-50 dark:bg-gray-800 p-3">
                <p className="text-xs font-medium text-gray-700 dark:text-gray-400">Formatos suportados:</p>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">MP3, WAV, M4A, OGG — até 25MB</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border dark:border-gray-700 bg-white dark:bg-gray-900 shadow-sm">
          ...
          : !transcriptions?.length ? (
            <div className="p-12 text-center">
              ...
              <p className="mt-3 text-sm text-gray-500 dark:text-gray-400">Nenhuma transcrição realizada</p>
            </div>
          ) : (
              ...
              <tbody className="divide-y dark:divide-gray-700">
                {transcriptions.map((t: any) => (
                  <tr key={t.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                    <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{t.user?.name}</td>
                    <td className="px-4 py-3 text-gray-600 dark:text-gray-400 max-w-xs truncate">{t.summary || t.originalText?.slice(0, 80)}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{t.durationSeconds ? `${t.durationSeconds}s` : '-'}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{t.model}</td>
                    <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{new Date(t.createdAt).toLocaleDateString('pt-BR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
