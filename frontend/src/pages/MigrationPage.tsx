  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500">Exportar dados do sistema ou importar de sistemas legados</p>
      </div>
      ...
      {tab === 'export' && (
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <button onClick={() => handleExport('all')} disabled={exportMutation.isPending} className="flex flex-col items-center gap-3 rounded-xl border bg-white p-6 shadow-sm transition-all hover:border-dental-300 hover:shadow-md disabled:opacity-50">
            <Database className="h-10 w-10 text-dental-600" />
            <div className="text-center"><p className="font-semibold text-gray-900">Exportar Tudo</p><p className="text-xs text-gray-500">Pacientes, agendamentos, procedimentos, financeiro</p></div>
          </button>
          <button onClick={() => handleExport('patients')} className="flex flex-col items-center gap-3 rounded-xl border bg-white p-6 shadow-sm transition-all hover:border-dental-300 hover:shadow-md">
            <Download className="h-10 w-10 text-blue-600" />
            <div className="text-center"><p className="font-semibold text-gray-900">Apenas Pacientes</p><p className="text-xs text-gray-500">Lista completa de pacientes cadastrados</p></div>
          </button>
          <button onClick={() => handleExport('appointments')} className="flex flex-col items-center gap-3 rounded-xl border bg-white p-6 shadow-sm transition-all hover:border-dental-300 hover:shadow-md">
            <Download className="h-10 w-10 text-purple-600" />
            <div className="text-center"><p className="font-semibold text-gray-900">Apenas Agendamentos</p><p className="text-xs text-gray-500">Histórico completo de agendamentos</p></div>
          </button>
          <button onClick={() => handleExport('all')} className="flex flex-col items-center gap-3 rounded-xl border bg-white p-6 shadow-sm transition-all hover:border-dental-300 hover:shadow-md">
            <FileJson className="h-10 w-10 text-green-600" />
            <div className="text-center"><p className="font-semibold text-gray-900">Formato JSON</p><p className="text-xs text-gray-500">Compatível com importação futura</p></div>
          </button>
        </div>
      )}
      ...
      {tab === 'import' && (
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="rounded-xl border bg-white p-6 shadow-sm">
            <h3 className="mb-4 font-semibold text-gray-900">Importar Dados</h3>
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700">Tipo de dados</label>
                <select value={importType} onChange={(e) => { setImportType(e.target.value); setImportResult(null); }} className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm">
                  <option value="patients">Pacientes</option>
                  <option value="procedures">Procedimentos</option>
                </select>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700">Arquivo JSON</label>
                <input type="file" accept=".json" onChange={handleFileUpload} className="mt-1 w-full rounded-lg border border-gray-200 px-3 py-2 text-sm file:mr-3 file:rounded file:border-0 file:bg-dental-50 file:px-3 file:py-1 file:text-sm file:font-medium file:text-dental-700 hover:file:bg-dental-100" />
                <p className="mt-1 text-xs text-gray-400">Formato: array de objetos ou {`{ "patients": [...] }`}</p>
              </div>
              {importData && (
                <div className="rounded-lg bg-gray-50 p-3">
                  <p className="text-xs text-gray-500">JSON carregado: {importData.length} caracteres</p>
                </div>
              )}
          ...
          <div className="rounded-xl border bg-white p-6 shadow-sm">
            <h3 className="mb-4 font-semibold text-gray-900">Resultado</h3>
            {importResult ? (
              ...
              <tbody className="divide-y">
                {history.map((h: any) => (
                  <tr key={h.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-gray-900">{h.entityType || '-'}</td>
                    <td className="px-4 py-3 text-gray-500">{h.fileName || '-'}</td>
                    <td className="px-4 py-3 text-right text-gray-900">{h.totalRows ?? '-'}</td>
                    ...
                    <td className="px-4 py-3 text-gray-500">{new Date(h.createdAt).toLocaleDateString('pt-BR')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
